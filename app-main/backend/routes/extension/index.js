...
router.post('/auth', authLimiter, async (req, res) => {
  try {
    ...
    // Previously only a null check existed; a 10,000-char "email" would hit MongoDB.
...
router.get('/tools/:toolId/credentials', verifyExtensionToken, async (req, res) => {
  try {
    ...
    // FIX: Validate toolId is a valid MongoDB ObjectId before querying.
    // A malformed ID causes Mongoose to throw a CastError that leaks a stack
