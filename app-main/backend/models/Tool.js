...
const sessionBundleSchema = new mongoose.Schema({
  ...
  // Last bundle update timestamp (distinct from Mongoose root updatedAt)
  bundleUpdatedAt: { type: Date, default: Date.now },
...
toolSchema.pre('save', async function() {
  ...
  if (this.isModified('sessionBundle.cookiesEncrypted') || 
    ...
    this.sessionBundle.bundleUpdatedAt = new Date();
